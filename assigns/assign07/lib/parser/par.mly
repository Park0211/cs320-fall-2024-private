...
%{                       (* 선언을 할떄 %를 씀  declartion *)
  open Utils
%}

%token <int> NUM                     (* 토큰을 선언   utils 를 가보면  NUM 보면 int 라고 나와있음  *)
%token <string> VAR
%token IF THEN ELSE LET IN FUN TRUE FALSE UNIT 
%token PLUS MINUS TIMES DIV MOD LT LTE GT GTE EQ NEQ AND OR
%token LPAREN RPAREN ARROW 
...
%start prog   (* start    expr 임  prog = expr *)
%type <Utils.prog> prog
...
prog:
  | expr EOF { $1 }   (* $1 은 expr 을 반환해라     (EOF 는 $2임) *)
...
expr2:
  ...
  |expr2 OR expr2 { Bop (Or, $1 , $3) }
  |expr3 { $1 }
  |expr3 expr3_tail{ List.fold_left (fun acc e -> App(acc, e)) $1 $2 }  (* extended = EBNF   *)
